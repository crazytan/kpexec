...
2. **Vault mutation requires a present human.** The vault (policies and secrets) can only be modified through kpexec's mutating subcommands, and every mutating subcommand must pass a user-presence check (Touch ID, or account password fallback) via LocalAuthentication before execution. The check cannot be satisfied from a terminal. Direct modification of the vault file is prevented because the vault master password is readable only by the genuine kpexec binary (see *Vault access control* below) — and KDBX4's authenticated encryption means a party without the master key can neither read nor undetectably modify the file.
...
- **Pinning covers bytes, not interpreter payloads.** `exe_sha256` closes the tampered-binary hole only for executables installed under an admin-owned immutable path. It does *not* cover what a script shim loads at run time: a pinned shim can still execute unpinned code elsewhere. Prefer self-contained binaries as policy targets. Explicit `--no-pin` opt-outs reopen the original risk and are flagged by `doctor`.
