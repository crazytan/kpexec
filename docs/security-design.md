...
1. **Secrets flow only into policy-shaped child processes.** A secret is injected — as an environment variable, into a single child process — only when the child's argv is constructed by kpexec itself from a policy-defined command template (absolute executable + fixed argument prefix) plus the agent's trailing arguments, and the executable's on-disk bytes match the policy's pinned SHA-256. The agent never supplies an executable. Any request that does not name an existing entry and command template, or whose pinned executable has changed, is rejected.
...
5. **Pinned executables** — the policy stores the SHA-256 of the canonical executable's bytes, computed at authoring time; `run` re-hashes immediately before exec and rejects on mismatch. Legitimate upgrades go through the Touch ID–gated `entry repin`. Unpinned commands require an explicit `--no-pin` opt-out and are flagged by `check` and `doctor`.
...
12. **Audit trail (advisory)** — every run, allowed or rejected, is logged with entry id, command name, canonical exe, hash of full argv, and exit code; never the secret or raw trailing args. The log file is same-user-writable, so it is evidence for the honest case, not tamper-proof.
...
- **Trailing-argument freedom is a credential-exfiltration path, not just target redirection.** Many CLIs accept endpoint-changing flags (`--hostname`, `--api-url`, registry or config-file flags); an allowed prefix plus one trailing flag can point the injected token at an attacker-controlled server. Author prefixes long enough to pin endpoint-relevant flags. Trailing-arg + cwd constraints are a reserved schema extension (see cli-design.md) and the next policy feature.
- **Pinning covers bytes, not interpreter payloads.** `exe_sha256` closes the tampered-binary hole for self-contained executables (like `gh`, a static Go binary). It does *not* cover what a script shim loads at run time: a pinned `node_modules/.bin/*` shim still executes unpinned JavaScript from `node_modules`. Prefer self-contained binaries as policy targets. Two smaller residuals: the hash-verify → exec window is a short TOCTOU race, and `--no-pin` opt-outs reopen the original risk (both flagged by `doctor`).
