...
- **Writable target binaries.** On a single-user Mac, essentially every realistic CLI location — including `/opt/homebrew/bin` — is writable by this threat model's adversary. A tampered "trusted" executable receives the env var and can exfiltrate it over the network. Binary hash pinning (`exe_sha256` + Touch ID–gated repin, reserved in the schema) is the real fix; until then `doctor` warns on project-local and user-writable executables.
